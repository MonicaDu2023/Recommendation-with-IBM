...
def user_item_train_test_split (df_joint, test_size=0.2, random_seed = 22):
    ...
    test_tmp = copy_df.sample(frac = test_size, random_state = random_seed)
    
    #eliminating taken data from rate_df so to form train
    train_tmp = copy_df.drop (index = test_tmp.index)
    
    #converting both into user_item format
    train_matrix = train_tmp.groupby(['user_id', 'article_id'])['interact'].sum ().unstack()
    test_matrix = test_tmp.groupby(['user_id', 'article_id'])['interact'].sum ().unstack()
